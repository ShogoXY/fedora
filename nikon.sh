...
sudo echo "ACTION==\"add\", SUBSYSTEM==\"usb\", ATTR{idVendor}==\"04b0\", ATTR{idProduct}==\"0448\", RUN+=\"gphoto2 --stdout --capture-movie | ffmpeg -i - -vcodec rawvideo -pix_fmt yuv420p -threads 0 -f v4l2 /dev/video0 \" " >> /etc/udev/rules.d/99-usb-script.rules
